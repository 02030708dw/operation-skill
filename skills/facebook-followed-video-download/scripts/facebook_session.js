...
async function inspect(ws) {
  const page=await engine.cdpCall(ws,{id:seq++,method:'Runtime.evaluate',params:{expression:`JSON.stringify({url:location.href, login:!!document.querySelector('input[name="pass"]'), challenge: /checkpoint|two_step_verification|recover|challenge/.test(location.pathname), loaded:document.readyState==='complete'})`,returnByValue:true}});
  ...
  if(data.challenge) return 'VERIFICATION_REQUIRED';
  if(data.login || /\/login/.test(new URL(data.url).pathname)) return 'LOGIN_REQUIRED';
  if(data.loaded && cookie && new URL(data.url).hostname==='www.facebook.com') return 'AVAILABLE';
  return 'COOLDOWN';
}
async function main() {
  ...
  try {
    ...
    if(input.action==='login') {
      await evaluate(`(()=>{const email=document.querySelector('input[name="email"]');const pass=document.querySelector('input[name="pass"]');if(!email||!pass)return false;email.value=${JSON.stringify(input.username)};pass.value=${JSON.stringify(input.password)};for(const e of [email,pass]){e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));}const submit=document.querySelector('button[name="login"],input[name="login"]');if(submit)submit.click();else pass.form.requestSubmit();return true;})()`);
      await sleep(8000);
      if(input.twoFactorSecret) {
        const code=totp(input.twoFactorSecret);
        await evaluate(`(()=>{const e=document.querySelector('input[name="approvals_code"],input[autocomplete="one-time-code"]');if(!e)return false;e.value=${JSON.stringify(code)};e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));const button=[...document.querySelectorAll('button,input[type="submit"],[role="button"]')].find(b=>/^(continue|submit|confirm|next|继续|繼續|ยืนยัน|ดำเนินการต่อ)$/i.test((b.innerText||b.value||'').trim()));if(button)button.click();return true;})()`);
        await sleep(7000);
      }
    }
    const state=await inspect(browser.ws);
