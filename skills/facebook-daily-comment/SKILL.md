...
| `MYT_MAX_RUNTIME` | `80` | 每台设备的最长执行秒数，确保常见的 120 秒外层超时前返回汇总 |
...
脚本默认在每台设备运行 80 秒时主动停止并输出部分结果。Hermes 调用 `terminal` 时应把外层命令超时设为至少 120 秒；不要仅通过增加 HTTP `--timeout` 来延长总执行时间。
...
```powershell
python "<SKILL_DIR>\scripts\facebook_daily_comment.py" --devices T1001 --count 1 --comment "good" --execute --force-restart
```
...
- **外层命令超时**：脚本有 80 秒内部截止时间并会先打印部分汇总。若汇总中 `sent_taps` 大于 `commented` 或状态为 `unverified-send`，先人工核对设备，不要因外层出现 exit 124 而重新执行，否则可能重复评论。
...
- **大批量评论性能**：单次 `--count 13` 并发两台设备容易在 80 秒内只完成 1-3 篇。建议分批补跑（每批 7-8 篇），或单台设备逐批执行并配合 `--force-restart`，每批可完成 3-7 篇。不要为省时间一次性并发大批量，否则反复补跑反而更慢。
