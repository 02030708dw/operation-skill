...
本地可枚举而服务器报账号编号缺失时，可用 `--impersonate chrome-131:macos-14` 显式固定 curl_cffi 的浏览器请求配置进行对照。该选项统一用于主页、视频解析及下载，不读取真实 Chrome Cookie，也不要求服务器是 macOS。默认仍使用提取器自身配置；报告 `requestClient` 记录本次选择，不因使用该选项而标记账号已登录。遇到明确验证码或限流仍按原规则停止，不轮换请求配置重试。
